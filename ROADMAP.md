...
- [ ] [UI-02 through UI-10] Deliver the approved scene-first, detailed-stylized nursery UI, authoritative presentation state, suggested editable care, shop previews, truthful companion controls, backup/recovery, and real-browser desktop/phone acceptance. UI-02 now has revision guards, a frontend store, and stream-frame validation; WebSocket lifecycle wiring, delta application, coherent server snapshots, and server-owned scheduling remain open. Full specification and ordered dependencies: `docs/r1-ui-upgrade.md`.
