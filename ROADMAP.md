...
- [ ] [UI-02 through UI-10] Deliver the approved scene-first, detailed-stylized nursery UI, authoritative presentation state, suggested editable care, shop previews, truthful companion controls, backup/recovery, and real-browser desktop/phone acceptance. UI-02 now has revision guards, a frontend store, stream-frame validation, reconnecting WebSocket transport, and plant summaries in stream projections; typed frame-to-store integration remains open. Full specification and ordered dependencies: `docs/r1-ui-upgrade.md`.
