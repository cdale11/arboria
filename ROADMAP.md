...
- [ ] [UI-03 through UI-10] Deliver the approved scene-first, detailed-stylized nursery UI, persistent shell, suggested editable care, shop previews, truthful companion controls, backup/recovery, and real-browser desktop/phone acceptance. UI-03 now has persistent navigation with in-place section switching, selected-plant-only care, plant-picker fallback, save-name draft retention, and a narrow-screen bottom-sheet presentation; native mobile validation remains open. UI-04 now has keyboard/pointer scene selection, bounded zoom/reset, wheel zoom, pointer pan, and pinch zoom; touch/native-browser validation remain open. UI-05 now has a server-backed bounded recommendation visible after inspection; editable preview execution and calibration evidence remain open. Full specification and ordered dependencies: `docs/r1-ui-upgrade.md`.
