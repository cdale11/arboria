...
- [ ] [UI-02 through UI-10] Deliver the approved scene-first, detailed-stylized nursery UI, authoritative presentation state, suggested editable care, shop previews, truthful companion controls, backup/recovery, and real-browser desktop/phone acceptance. UI-02 now has revision guards, a frontend store, stream-frame validation, and reconnecting WebSocket transport; typed delta application and coherent server snapshot integration remain open. Full specification and ordered dependencies: `docs/r1-ui-upgrade.md`.
