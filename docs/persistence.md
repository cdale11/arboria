...
Implemented R1 checkpoints are immutable metadata-only generations under `checkpoints/<checkpoint_id>/` with `manifest.json` and `state.json`. They record the current world/timeline/request epoch, clock, tick/revision state, and receipt count, then register a complete checkpoint row and active checkpoint ID in SQLite. Metadata-only named snapshots can point to these checkpoints through the `snapshot_names` table. Metadata-only restore validates the manifest ID, `state.json` size, and SHA-256 before restoring clock/tick state with a fresh timeline and request epoch. Checkpoints do not yet contain biology, economy, learner arrays, RNG streams, pending queues, export/import, or retention behavior.
