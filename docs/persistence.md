...
Implemented R1 checkpoints are immutable metadata generations under `checkpoints/<checkpoint_id>/` with `manifest.json` and `state.json`. They record the current world/timeline/request epoch, clock, tick/revision state, receipt count, purpose (`manual` or `autosave`), nursery schema version, nursery organs, nursery water zones, nursery nutrient zones, nursery species mapping, nursery economy (cash and demand), protected plant IDs, and nursery reservoir, then register a complete checkpoint row and active checkpoint ID in SQLite. Metadata-only named snapshots can point to these checkpoints through the `snapshot_names` table. Metadata-only autosave writes checkpoints after the configured interval when authenticated world status is observed and stores the last autosave timestamp. Restore validates the manifest ID, `state.json` size, and SHA-256 before restoring clock/tick/nursery state with a fresh timeline and request epoch. Nursery payloads migrate through the versioned `load_nursery_state` entry: schema versions 1–4 load with starter fallbacks, newer stamps are rejected. Startup removes hidden `*.tmp` checkpoint directories left by interrupted generation writes before opening mutable metadata, then validates the active checkpoint if one is recorded and recovers nursery biology from it. When the active checkpoint fails validation, startup walks the manifest parent chain for the first loadable generation, repoints the active checkpoint at it (or boots starter state when nothing loads), and logs the recovery; corrupt generations are skipped, never repaired. Checkpoint creation is atomic through a hidden `.tmp` generation published by a single rename. Current-domain export/import is available through `/api/v1/saves/export` and `/api/v1/saves/import` as a base64 zip containing `arboria-export.json`, `manifest.json`, and `state.json`; imports reject malformed archives, extra entries, oversize entries, and invalid checkpoint hashes before registration/restore. Checkpoints do not yet contain learner arrays, RNG streams, pending queues, or retention behavior.
