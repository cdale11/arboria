...
Current R1 implementation uses `/api/v1/commands` for bounded, receipt-backed clock commands (`clock.pause`, `clock.resume`, `clock.set_speed`) and nursery watering (`nursery.water` with `plant_id` and bounded `water_kg`). It rejects unknown envelope fields, invalid UUIDs, unsupported schema versions, nonfinite payload numbers, oversized/nested payloads, stale world/timeline/request-epoch values, unknown command kinds, and invalid clock payloads. Receipts are persisted in SQLite and duplicate command IDs within the active timeline/epoch return the original receipt. Horticultural commands, queued tick execution, receipt expiry/rotation policy, and checkpoint reconciliation are not implemented yet.
