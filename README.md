...
**Authenticated living-nursery baseline with two ticking starter plants (greenhouse basil and outdoor oak from a six-species provisional catalog), root-zone water with finite reservoir watering, substrate N/P/K uptake with deficiency stress, damage, and death, shop economy with cash purchases and demand-limited plant sales, generated 2.5D SVG nursery projection, usable browser controls for inspection, care, shop, saves, clock, current-domain export/import, plant sale protection, and a baseline caretaker companion that can propose validated watering actions, plus biology checkpoints. Fertilizer items, species calibration, neural learning, learner-inclusive export/import, and real touch-device validation have not been implemented yet.** The documents in this repository define intended behavior and delivery gates; they are not evidence that the described features work. See [ROADMAP.md](ROADMAP.md) for implementation status.
...
The current server exposes authenticated world metadata at `/api/v1/world` and a simulation clock at `/api/v1/clock`, with CSRF-protected pause, resume, and speed controls. It also exposes the command endpoint at `/api/v1/commands` for bounded, deduplicated, receipt-backed clock, nursery-watering, plant protection (`nursery.protect`, `nursery.unprotect`), and shop (`shop.buy_water`, `shop.buy_plant`, `shop.sell_plant`) commands, `/api/v1/stream` for authenticated bounded WebSocket snapshot/ping messages, `/api/v1/saves/checkpoint` for manual checkpoint creation, `/api/v1/saves` plus `/api/v1/saves/named` for named save listing/creation, `/api/v1/saves/restore` for restore, `/api/v1/saves/export` and `/api/v1/saves/import` for current-domain base64 zip backups, and `/api/v1/plants` plus `/api/v1/plants/{id}` for living-nursery inspection. Protected plants cannot be sold until unprotected. Checkpoints now include starter-plant biology, root-zone water, substrate N/P/K, species mapping, economy cash/demand, protected plant IDs, and reservoir state in addition to metadata. World identity, request epoch, command receipts, clock state, tick count, world revision, active checkpoint ID, autosave timestamp, and named snapshots are stored in `world.sqlite3`; process restart keeps the world UUID, records a new timeline UUID/request epoch, recovers nursery biology from the active checkpoint when present, and resumes from the last saved clock/tick state without wall-clock catch-up. Learner-inclusive export/import, fertilizer items, price forecasting, species calibration, companion behavior, and remaining horticultural command execution are not implemented yet.

## Saves and backups

The default data directory is `./var/`, relative to the repository root, overridable with `ARBORIA_DATA_DIR`. Autosaves, named snapshots, restore, and current-domain export/import include implemented biology, shop state, protected plant IDs, clock, and world-loop state. Learner RNG state, training data, model weights, optimizer state, companion preferences, and rule versions are not included because those systems are not implemented yet. Live database copying is not a supported backup procedure; use the export control or `/api/v1/saves/export` for the implemented-domain archive. See [persistence](docs/persistence.md).
...
- [UI-01 design frames](docs/ui-design-frames.md): review draft for responsive layouts and interaction states.
